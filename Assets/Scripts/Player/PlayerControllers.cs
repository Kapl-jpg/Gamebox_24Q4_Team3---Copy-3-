...
public class PlayerControllers : MonoBehaviour
{
    ...
    private void CamMovement()
    {
        ...
        if (_inputManager.IsRotatingMirror() && _inputManager.IsUsingMirror())
            return;
        _xRotation -= mouseY * MouseSensitivity.Instance.GetSensValue() * Time.smoothDeltaTime;
        _xRotation = Mathf.Clamp(_xRotation, UpperLimit, BottomLimit);

        Camera.localRotation = Quaternion.Euler(_xRotation, 0, 0);

        _playerRigidbody.MoveRotation(_playerRigidbody.rotation *
                                      Quaternion.Euler(0, mouseX * MouseSensitivity.Instance.GetSensValue() * Time.smoothDeltaTime, 0));
    }
